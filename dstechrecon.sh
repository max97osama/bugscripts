...
while IFS= read -r TARGET; do
    ...
    SCAN_PORTS="$CURATED_PORTS"

    if command -v naabu >/dev/null 2>&1; then
        NAABU_OUT="/tmp/naabu_tech_$$.json"
        echo "[*] naabu pre-scan on curated port list for $TARGET..."
        timeout 180 naabu -host "$TARGET" -p "$CURATED_PORTS" \
            -rate 200 -c 25 -silent -json -o "$NAABU_OUT" 2>/dev/null

        FOUND_PORTS=$(python3 -c "
import json
ports = set()
try:
...
")
        ...
        if [ -z "$FOUND_PORTS" ]; then
            echo "[*] naabu found no open ports on $TARGET out of the curated list, skipping nmap."
            echo "[*] No open ports found by naabu pre-scan." >> "$RAW_ALL"
            sleep 3
            continue
        fi

        echo "[+] naabu found open ports on $TARGET: $FOUND_PORTS"
        SCAN_PORTS="$FOUND_PORTS"
    else
        echo "[-] naabu not found, nmap will discover ports itself from the curated list (slower)."
    fi
    ...
    if [ $? -eq 124 ]; then
        echo "[!] $TARGET nmap scan hit its time limit, killed"
    fi
...
while IFS= read -r TARGET; do
    ...
    echo "=== SECURITY HEADER AUDIT: $TARGET ===" >> "$RAW_ALL"
    HDR=$(curl -skI --max-time 12 -A "Mozilla/5.0" "$TARGET")
    for h in "strict-transport-security" "content-security-policy" "x-frame-options" "x-content-type-options" "referrer-policy" "permissions-policy"; do
        if echo "$HDR" | grep -qi "^$h:"; then
            echo "  [OK]   $h: $(echo "$HDR" | grep -i "^$h:" | tr -d '\r' | head -1)" >> "$RAW_ALL"
        else
            echo "  [MISSING] $h" >> "$RAW_ALL"
        fi
    done
    echo "  Cookie flags:" >> "$RAW_ALL"
    echo "$HDR" | grep -i "^set-cookie:" | tr -d '\r' | while read -r c; do
        CK=$(echo "$c" | awk '{print $2}' | cut -d= -f1)
        if echo "$c" | grep -qi httponly; then HK="HttpOnly=YES"; else HK="HttpOnly=NO"; fi
        if echo "$c" | grep -qi secure; then SK="Secure=YES"; else SK="Secure=NO"; fi
        echo "    $CK [$HK $SK]" >> "$RAW_ALL"
    done

    echo "=== TLS / JARM: $TARGET ===" >> "$RAW_ALL"
    if command -v tlsx >/dev/null 2>&1; then
        tlsx -host "$(echo "$TARGET" | sed 's|https\?://||;s|/.*$||')" -port 443 -silent -jarm 2>/dev/null >> "$RAW_ALL"
    fi
...

{
    echo ""
    echo "========================================================================="
    echo " [dstechrecon] TECH / SERVICES / OS $(date '+%Y-%m-%d %H:%M:%S')"
    echo "========================================================================="
    cat "$TECHSTACK_OUT" 2>/dev/null
    echo ""
